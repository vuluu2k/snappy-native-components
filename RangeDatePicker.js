...
const subtractDate = days => dayjs().subtract(days, 'day').startOf('day');
const formatMarked = day => dayjs(day).format(formatDateMarked);
const formatDateValue = day => dayjs(day).valueOf();
...
export default class RangeDatePicker extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      showModalStartDate: props.visibleStart,
      showModalEndDate: props.visibleEnd,
      markedDates: {},
      option: undefined,
  ...
  onSelectCheckBox = rangeDays => {
    ...
    if (rangeDays === 'seven_days') {
      ...
      this.mounted &&
        this.setState({ startDate: formatMarked(before07day), endDate: formatMarked(endToDay), keyVn: this.cvtLabel(rangeDays), keyEng: rangeDays });
    }
    if (rangeDays === 'month') {
    ...
    if (rangeDays === 'thirty_days') {
      ...
      this.mounted &&
        this.setState({ startDate: formatMarked(before30Day), endDate: formatMarked(endToDay), keyVn: this.cvtLabel(rangeDays), keyEng: rangeDays });
    }
    if (rangeDays === 'sixty_days') {
      ...
      this.mounted &&
        this.setState({ startDate: formatMarked(before60Day), endDate: formatMarked(endToDay), keyVn: this.cvtLabel(rangeDays), keyEng: rangeDays });
    }
    if (rangeDays === 'to_day') {
    ...
    if (rangeDays === 'last_week') {
      ...
      this.mounted &&
        this.setState({ startDate: formatMarked(lastWeek), endDate: formatMarked(endLastWeek), keyVn: this.cvtLabel(rangeDays), keyEng: rangeDays });
    }
    if (rangeDays === 'last_month') {
      ...
      this.mounted &&
        this.setState({ startDate: formatMarked(lastMoth), endDate: formatMarked(endLastMonth), keyVn: this.cvtLabel(rangeDays), keyEng: rangeDays });
    }
    if ((rangeDays === 'custom_day', rangeDays)) {
      this.mounted && this.setState({ openPicker: true, keyVn: this.cvtLabel(rangeDays), keyEng: rangeDays });
    }
  };

  renderItemDateBox = (item, option) => {
    return (
  ...
  renderModalDatePicker = (showModal, currentDate, markedDates) => {
    return (
  ...
  resetFilter = () => {
    ...
    if (defaultWeek) {
      this.mounted &&
        this.setState({ startDate: formatMarked(week), endDate: formatMarked(endToDay), selectCheckBox: undefined, openPicker: openPicker });
    } else {
      this.mounted && this.setState({ startDate: formatMarked(before30Day), endDate: formatMarked(endToDay), openPicker: openPicker });
    }
  };
        ...
        {showButton && (
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <TouchableOpacity
              ...
              onPress={() => onSubmit({ keyVn: keyVn, keyEng: keyEng }, formatDateValue(startDate), formatDateValue(endDate))}
